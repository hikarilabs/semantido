...
Some checks are free because semantido rises at sync time: a `time_dimension` naming a column that doesn't exist, or isn't temporal, fails `sync_semantic_layer()` — so any test that builds the layer catches it.
...
Pin the semantido version if you plan to snapshot exports. Exporter output is alpha and will move; the authoring surface is what's stable.
...
- name: Fail if the committed model is stale
  run: git diff --exit-code model.ossie.yaml
```

The same pattern as committed lockfiles or generated clients. Byte-identical output is what makes it work — without determinism the file churns every run and everyone learns to ignore it.
...
The layer is a pure function of the model files. It cannot change without a deployment. Rebuilding per request is a waste.
