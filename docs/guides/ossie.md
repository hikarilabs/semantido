---
title: Apache Ossie (Open Semantic Interchange)
description: Export a vendor-neutral semantic model — how the mapping works, what's lossy, and why you'd want it.
---

# Apache Ossie (Open Semantic Interchange)

[Apache Ossie](https://open-semantic-interchange.org) is a vendor-neutral format for exchanging semantic models. semantido exports to it, which means: author once in code, hand the model to whatever consumes it.
...
to_ossie_yaml(layer, model_name="commerce", path="model.ossie.yaml")
to_ossie_dict(layer, model_name="commerce")     # no PyYAML needed
```

Only YAML serialisation needs PyYAML. `to_ossie_dict()` works on the core installation — useful if you're handing the structure to a JSON API or your own writer.
...
Apache Ossie is the bet that you shouldn't have to make that choice permanent. Author in code, export to the interchange format, let the consumer of the month read it.

The honest issue: **Apche Ossie is pre-1.0.** semantido targets spec version `0.2.0.dev0`. Ecosystem support is early. This is a position on where things are going, not a description of where they are.

## The mapping

| semantido                                                   | Apache Ossie                                             |
|-------------------------------------------------------------|----------------------------------------------------------|
...
Apache Ossie has no first-class home for some of what semantido captures, so it goes into vendor extensions:
...
The Apache Ossie exporter does something the other exporters don't: it takes a position on which temporal columns matter.
...
That demotion is active, not passive. Saying nothing about `created_at` is not neutral — a model looking for a date column will find it. See [Modeling time](modelling-time.md) for overriding the pattern.
...
to_ossie_yaml(
    ...
    path="model.ossie.yaml",              # also write to disk
)
```
...
`to_ossie_yaml` returns the YAML text whether you pass `path`.

## Commit the output

The export is [deterministic](../concepts/determinism.md), so a committed `model.ossie.yaml` produces no diff until a definition actually changes — at which point the diff *is* the semantic change, isolated and reviewable. That property is most of the why the format is worth having in the repo at all.
