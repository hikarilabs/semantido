...
| **Apache Ossie YAML export** | ⚠️ Works; tracks a pre-1.0 spec (`0.2.0.dev0`). [Guide](../guides/ossie.md) |
...
**Status: early, separate package.** Check the [Hikari Labs GitHub org](https://github.com/hikarilabs) for the current state before planning around it. Do not assume the interface below is final.
...
Via [Apache Ossie](../guides/ossie.md), with an issue worth taking seriously: Apache Ossie is pre-1.0 and ecosystem support is early. `semantido` puts `privacy_level`, `sample_values`, `time_grain`, and cardinality into `custom_extensions` under the `SEMANTIDO` vendor — a consumer that doesn't read extensions silently drops them.
